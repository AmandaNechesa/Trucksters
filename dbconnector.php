...
class dbconnector
{
    ...
    function __construct()
    {
        ...
        try {
            $this->conn = new PDO("mysql:host=localhost;dbname=trucksters", 'root', '');
    ...
    {
        $sql = "SELECT * FROM truckstersusers WHERE email=?";
        ...
        if ($res->rowCount() > 0) {
            ...
            if ($password === $passwordconf) {
                ...
                $insert = "INSERT INTO truckstersusers (name,email,phonenumber,location,password,hash)VALUES (?,?,?,?,?,?)";
    ...
    {
        $sql = "SELECT * FROM truckstersusers WHERE email=? AND activated=?";
        $res = $this->conn->prepare($sql);
    ...
    {
        $sql = "UPDATE truckstersusers SET activated=? WHERE hash=?";
        $res = $this->conn->prepare($sql);
    ...
    function newpost()
    {
        ...
        if (in_array($_FILES['avatar-file']['type'], $allowedfiles)) {
            if ($_FILES['avatar-file']['size'] < 8000000000) {
                ...
                $sql = "INSERT INTO truckstersitems(pic,name,quantity,uploaderid,categoryid,price,location)VALUES(?,?,?,?,?,?,?)";
...
//todo continue with posting
    ...
    function fetchemall()
    {
        ...
        $sql = "SELECT * FROM truckstersitems WHERE uploaderid=" . $_SESSION['adm id'];
    ...
    {
        $sql = "SELECT * FROM truckstersitems ";
        $res = $this->conn->query($sql);
        return $res;
    }
    ...
    {
        $sql = "SELECT * FROM truckstersitems where categoryid=$factor";
        $res = $this->conn->query($sql);
        return $res;
    }
    ...
    {
        $sql = "DELETE FROM truckstersitems WHERE id='$id'";
        $sq = "SELECT * FROM truckstersitems WHERE id=?";
